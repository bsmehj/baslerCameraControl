/*
 * 说明：此类对window注册表文件操作进行相关封，方便后续工程中直接直接使用
 * 作者：huangjun
 * 时间：2020-12-03
 */
...
namespace BaslerCameraCtrl.UserMethodClass
{
    ...
    class RegisterOperaterClass
    {
        ...
        public RegisterOperaterClass(string subKey, RegDomain regDomain)
        {
            ///设置注册表项名称  
            _subkey = subKey;
            ///设置注册表基项域  
        ...
        /// 虚方法，子类可进行重写  
        /// 例子：如regDomain 是HKEY_CLASSES_ROOT，subkey 是software\\higame\\，
        /// 则将判断HKEY_CLASSES_ROOT\\software\\higame\\注册表项是否存在  
        /// </summary>  
        /// <param name="subKey">注册表项名称</param>  
        /// <param name="regDomain">注册表基项域</param>  
        /// <returns>返回注册表项是否存在，存在返回true，否则返回false</returns>  
        ...
        private void ReadRegeditCodeTemp()
        {
            // 此代码为演示使用
        ...
        private void WriteRegeditCodeTemp()
        {
            // 此代码为演示使用
            // 创建了一个test_test子键并有三个子健
            RegistryKey regkeySetKey = Registry.CurrentUser.
                OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", true).CreateSubKey("test_test"); 
